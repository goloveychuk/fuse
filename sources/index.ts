...
class FuseInstaller implements Installer {
  ...
  private realLocatormetadata: Map<LocatorHash, string | null> = new Map();
  ...
  async installPackage(
    ...
    const checksum = fetchResult.checksum
      ...
      : null;
    this.realLocatormetadata.set(realLocator.locatorHash, checksum);
  ...
  async finalizeInstall() {
    ...
    const nmPath = ppath.join(this.opts.project.cwd, NODE_MODULES);
    await unmountFuse(nmPath);
    ...
    //   realLocatormetadata: this.realLocatormetadata,
    ...
    let fuseState = buildFuseTree(locationTree, binSymlinks);
    //@ts-ignore
    fuseState = Object.values(fuseState.roots)[0]
    // console.log(tree)
