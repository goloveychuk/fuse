...
let getattr:
    @convention(c) (
        UnsafePointer<Int8>?, UnsafeMutablePointer<stat>?, UnsafeMutablePointer<fuse_file_info>?
    ) -> Int32 = { path, stbuf, file_info in
        ...
        if let filename = components.last {
            ...
            if dummyEntries.contains(filenameStr) {
                if filenameStr == "directory1" || filenameStr == "directory2" || filenameStr == "."
                    || filenameStr == ".."
                {
                    stbuf.pointee.st_mode = UInt32(clibfuse.S_IFDIR | 0o755)  // Specify module
                    stbuf.pointee.st_nlink = 2
...
class DummyFileSystem: NSObject {
    let val = 32  // This class can be used
    deinit {
        print("DummyFileSystem deinit")
    }
}
// MARK: - Main Function

func shit() {
    let dummyFS = DummyFileSystem()

    let unmanaged = Unmanaged.passRetained(dummyFS)
    let pointer = unmanaged.toOpaque()
    let pointerAsUInt64 = UInt64(Int(bitPattern: pointer))
    print("Pointer to DummyFileSystem: \(pointerAsUInt64)")


    DispatchQueue.global().asyncAfter(deadline: .now() + 5.0) {
        let restoredPointer = UnsafeRawPointer(bitPattern: Int(pointerAsUInt64))!
        print("Executing async operation after 5 seconds...")
        let restoredObject = Unmanaged<DummyFileSystem>.fromOpaque(restoredPointer)
            .takeRetainedValue()
        print("Restored object value: \(restoredObject.val)")
    }
...
func main() {
    ...
    let result = fuse_main_real(
        Int32(args.count), &cArgs, &operations, MemoryLayout<fuse_operations>.size, nil)
