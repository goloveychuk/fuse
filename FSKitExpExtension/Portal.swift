...
final class PortalDirFSItem: FSItem, FSItemProtocol, WriteFSItemProtocol {
    ...
    func getChildren() throws -> [(FSFileName, FSItemProtocol)] {
        ...
        while let entry = readdir(dir) {
            ...
            if type == .directory {
                ...
                if itemFD >= 0 {
                    entries[name] = PortalDirFSItem(fileId: nextID, parentId: fileId, dirFD: itemFD, dirPath: childPath)
                }
            } else {
                // Open file
                itemFD = openat(dirFD, name, O_RDONLY, 0)
                if itemFD >= 0 {
                    entries[name] = PortalFileFSItem(fileId: nextID, parentId: fileId, fd: itemFD, path: childPath, itemType: type)
                }
            }
    ...
    func getChild(name: FSFileName) throws -> FSItemProtocol? {
        ...
        let nextID = fileId.advance(by: UInt64(nameStr.hash & 0x7FFFFFFF))
        ...
        if (statBuf.st_mode & S_IFMT) == S_IFDIR {
            ...
            if newFD >= 0 {
                return PortalDirFSItem(fileId: nextID, parentId: fileId, dirFD: newFD, dirPath: childPath)
            }
        } else if (statBuf.st_mode & S_IFMT) == S_IFREG {
            type = .file
            let newFD = openat(dirFD, nameStr, O_RDONLY, 0)
            if newFD >= 0 {
                return PortalFileFSItem(fileId: nextID, parentId: fileId, fd: newFD, path: childPath, itemType: type)
            }
        } else if (statBuf.st_mode & S_IFMT) == S_IFLNK {
            type = .symlink
    ...
    func getAttributes() throws -> FSItem.Attributes {
        ...
        attributes.mode = UInt32(statBuf.st_mode & 0o777) | S_IFDIR
        ...
        attributes.birthTime = createTime
    ...
    func setAttributes(newAttributes: FSItem.SetAttributesRequest) throws -> FSItem.Attributes {
        ...
        // Handle ownership
        if newAttributes.isValid(FSItem.Attribute.uid) || newAttributes.isValid(FSItem.Attribute.gid) {
        ...
        // Handle timestamps
        if newAttributes.isValid(FSItem.Attribute.accessTime) || newAttributes.isValid(FSItem.Attribute.modifyTime) {
            var times = [timespec](repeating: timespec(tv_sec: 0, tv_nsec: UTIME_OMIT), count: 2)
            
            if newAttributes.isValid(FSItem.Attribute.accessTime) {
                times[0] = timespec(tv_sec: newAttributes.accessTime.tv_sec, tv_nsec: newAttributes.accessTime.tv_nsec)
            }
            
            if newAttributes.isValid(FSItem.Attribute.modifyTime) {
                times[1] = timespec(tv_sec: newAttributes.modifyTime.tv_sec, tv_nsec: newAttributes.modifyTime.tv_nsec)
            }
    ...
    func createSymbolicLink(
        ...
        if itemFD >= 0 {
            let item = PortalFileFSItem(fileId: nextID, parentId: fileId, fd: itemFD, path: childPath, itemType: .symlink)
    ...
    func createItem(
        ...
        case .directory:
            ...
            let dirItem = PortalDirFSItem(fileId: nextID, parentId: fileId, dirFD: itemFD, dirPath: childPath)
        ...
        case .file:
            ...
            let fileItem = PortalFileFSItem(fileId: nextID, parentId: fileId, fd: fd, path: childPath, itemType: .file)
...
final class PortalFileFSItem: FSItem, FSItemProtocol, WriteFSItemProtocol {
    ...
    init(fileId: FSItem.Identifier, parentId: FSItem.Identifier, fd: Int32, path: String, itemType: FSItem.ItemType) {
        self.fileId = fileId
        self.parentId = parentId
        self.fd = fd
        self.path = path
    ...
    func getAttributes() throws -> FSItem.Attributes {
        ...
        switch itemType {
        case .file:
            attributes.mode = UInt32(statBuf.st_mode & 0o777) | S_IFREG
        case .symlink:
            attributes.mode = UInt32(statBuf.st_mode & 0o777) | S_IFLNK
        default:
    ...
    func createLink(to name: FSFileName, inDirectory: FSItem) throws -> FSFileName {
        guard let destName = name.string,
              let destDir = inDirectory as? PortalDirFSItem else {
            throw fs_errorForPOSIXError(POSIXError.EINVAL.rawValue)
        }
        
        // Create hard link from this file to the destination
        if linkat(AT_FDCWD, path, destDir.dirFD, destName, 0) != 0 {
            throw fs_errorForPOSIXError(errno)
        }
